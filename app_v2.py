...
def match_videos():
    ...
    result_box.insert(tk.END, "Đang extract frame...\n")
    ...
    result_box.insert(tk.END, "Tính pHash original...\n")
    ...
    for i, frame in enumerate(review_frames):
        ...
        result_box.insert(
            tk.END,
            f"{review_ts[i]:.2f}s → {original_ts[final_idx]:.2f}s | pHash:{best_dist} SSIM:{best_ssim:.2f} CLIP:{c_score:.2f}\n"
        )
...
def save_results(results):
    ...
    with open(path, "w") as f:
        for r in results:
            f.write(f"{r[0]:.2f}s -> {r[1]:.2f}s | pHash:{r[2]} SSIM:{r[3]:.2f} CLIP:{r[4]:.2f}\n")

# ===== GUI =====
root = tk.Tk()
root.title("Video Matching Tool (CLIP + pHash + SSIM)")
...
result_box = tk.Text(root, height=20)
result_box.pack()
